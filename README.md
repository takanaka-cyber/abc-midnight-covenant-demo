...
`codex/live2d-rig-poc` では、公式Cubism形式ではないWebGL 2.5Dリグを質問画面へ埋め込んでいます。まばたき、前髪、ロングコート裾、呼吸、胸部の局所スプリングを別々に動かし、失敗時とreduced motionでは既存の静止画へ戻します。実装・実測は `live2d/` を参照してください。
